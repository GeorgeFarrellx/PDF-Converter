...
def _infer_period_business(blob: str) -> Tuple[Optional[_dt.date], Optional[_dt.date]]:
    ...
    m = re.search(
        r"your account summary for\s+(\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4})\s+to\s+(\d{1,2}\s+[A-Za-z]{3,9}\s+\d{4})",
        blob, re.IGNORECASE
    )
    if m:
...
def _extract_transactions_business(pdf_path: str) -> List[Dict]:
    ...
    for raw in lines:
        ...
        if "previous statement balance" in low or "current statement balance" in low:
            continue
